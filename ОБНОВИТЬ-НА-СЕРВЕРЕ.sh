...
# --force: обновляться, даже если в папке системы есть правки.
#
# Обычно обновление от таких правок отказывается — и правильно: кто-то мог
# чинить что-то прямо на живом сервере, и стереть это молча значит потерять
# чужую работу. Но бывает случай, когда правок нет, а git всё равно считает
# файл изменённым: если в хранилище лежит файл с чужими переводами строк,
# «вернуть как было» не помогает — возвращается тот же неверный вариант.
# Круг замыкается: обновление не идёт из-за файла, а файл чинится только
# обновлением. Флаг этот круг разрывает.
#
# Имя флага латиницей намеренно: его набирают в консоли сервера, где русской
# раскладки нет.
FORCE=0
for arg in "$@"; do
  [ "$arg" = "--force" ] && FORCE=1
...
if ! sudo -u "$APP_USER" git diff --quiet || ! sudo -u "$APP_USER" git diff --cached --quiet; then
  if [ "$FORCE" = 1 ]; then
    echo "   ВНИМАНИЕ: в папке есть правки, и они будут стёрты (запрошено --force):"
    sudo -u "$APP_USER" git -c core.quotepath=false status --porcelain | sed 's/^/     /'
  else
  ...
  Если «вернуть как было» не помогает и файл остаётся изменённым — дело
  в переводах строк, и лечится это только свежей версией:
      bash $0 $APP_DIR --force"
  fi
fi
...
# --force здесь не «на всякий случай»: без него git откажется перезаписывать
# изменённый файл, и обновление встанет ровно там же, где встало без флага.
if [ "$FORCE" = 1 ]; then
  sudo -u "$APP_USER" git checkout --quiet --force -B "$BRANCH" "origin/$BRANCH"
else
...
# Демо-версия (ДЕМО-И-УВЕДОМЛЕНИЯ.sh) работает на том же коде — перезапускаем
# и её, иначе знакомящиеся с системой и проверяющий Apple видели бы прошлую
# версию. Её сбой на рабочую систему не влияет, поэтому только предупреждаем.
if systemctl is-enabled --quiet asher-demo 2>/dev/null; then
  systemctl restart asher-demo || echo "   ВНИМАНИЕ: демо-версия не перезапустилась: journalctl -u asher-demo -n 50"
fi
