...
const routes = [
  {
    ...
    handler: ({ query, session }) => {
      ...
      if (!видитВсё(session.role)) {
        ...
        /*
         * Тревоги — только владельцу и только на Главной.
         *
         * Копии ломаются тихо: кончилось место, сменились права, отвалился диск.
         * Раньше об этом знала строка в чёрном окне, куда никто не смотрит,
         * и выяснилось бы это в тот единственный день, когда копия понадобилась.
         * Главную владелец открывает каждый день — здесь он это и увидит.
         *
         * Продавцу тревоги не показываем: сделать он с ними ничего не может,
         * а пугать человека за прилавком незачем.
         */
        const копии = require('./settings').состояниеКопий();
        const тревоги = [];
        if (копии.backup_error) {
          тревоги.push({
            уровень: 'плохо',
            что: 'Резервные копии не делаются',
            почему: копии.backup_error,
            делать: 'Настройки → Безопасность. Пока не починено, потеря компьютера = потеря всего.',
          });
        } else if (копии.backup_stale) {
          тревоги.push({
            уровень: 'плохо',
            что: копии.last_backup
              ? `Последняя копия ${копии.backup_hours_ago} ч назад`
              : 'Копий ещё ни одной',
            почему: 'Копия должна делаться раз в сутки',
            делать: 'Настройки → Безопасность → «Скачать резервную копию».',
          });
        }
        if (копии.disk_low) {
          тревоги.push({
            уровень: 'внимание',
            что: `На диске осталось ${копии.disk_free_mb} МБ`,
            почему: 'Когда место кончится, копии перестанут делаться, а система — записывать продажи',
         ...
         * системы, а незакрытое дело владельца — но забытая аренда завышает
         * прибыль в отчёте ровно на свою сумму, поэтому место ей здесь же.
         */
        for (const п of require('./finance').неЗаписаны()) {
          тревоги.push({
            уровень: 'внимание',
            что: `Не записан расход: ${п.подпись}`,
            почему: `Обычно ${п.amount} сом, ${п.day_of_month}-го числа. Пока не записан, прибыль в отчёте завышена`,
            делать: 'Финансы → Постоянные расходы → «Записать».',
          });
        }
        if (тревоги.length) result.тревоги = тревоги;
      }
      return result;
    },
  },
