...
function createSaleTx(body, session, opts = {}) {
  ...
  // Разрешение владельца на этот чек — если продавец его попросил и получил.
  const разрешение = body.discount_request_id
    ? require('./discounts').разрешениеДляПродажи(body.discount_request_id, session) : null;
  ...
  for (const it of items) {
    ...
    /*
     * Сравниваем в сомах, как касса (price × предел / 100), а не в процентах:
     * проценты округлялись до сотых, и на изделии за 673 500 скидка 100 сом
     * давала «0,01%» — проходила даже при пределе 0%. Копейка запаса — от
     * округления суммы, а не поблажка.
     */
    if (discount > Math.round(price * пределПроцентов) / 100 + 0.009) {
      ...
      // Разрешение действует на это изделие и не больше разрешённой скидки.
      const поРазрешению = Boolean(разрешение) && разрешение.скидки.has(p.id)
        && discount <= разрешение.скидки.get(p.id) + 0.01;
      if (!видитВсё(session.role) && !этоЦенаКомплекта && !поРазрешению) {
   ...
   *
   * Сюда попадают продажи владельца и продажи продавца по разрешению
   * владельца — тогда в строке сказано, кто разрешил. Строка нужна не ради
   * недоверия, а ради памяти: через полгода
...
module.exports = { routes, пределСкидки };
