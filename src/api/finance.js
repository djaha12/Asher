...
const routes = [
  ...
  },
  {
    ...
    handler: ({ query }) => {
      ...
      // «Покупка лома» — золото, принятое в зачёт покупки: это не расход,
      // а запас металла, и выручку чека она не уменьшает.
      const EXCLUDED_EXPENSES = ['Возврат покупателю', 'Закупка товара', 'Оплата поставщику', 'Покупка лома'];
