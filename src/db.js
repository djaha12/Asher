...
function migrate() {
  ...
  /*
   * Из какой поставки пришло изделие.
   *
   * Раньше товар заводился по одному, а долг перед поставщиком вбивался
   * вторым заходом руками — и связи между ними не было никакой. Через месяц
   * на вопрос «что было в той накладной на 3 миллиона» ответить было нечем,
   * а сумма долга держалась на том, что кто-то не ошибся при сложении.
   */
  addColumn('products', 'supplier_op_id',
    'INTEGER REFERENCES supplier_ops(id) ON DELETE SET NULL');

  /*
   * Прошли ли эти деньги через кассовый ящик.
   *
   * Способа оплаты для сверки кассы оказалось мало. При обмене клиент
   * «платит» старым изделием: в истории платежей это законная строка —
   * человек действительно рассчитался, — но живых денег в ящик не легло
   * ни одного сома. Пока сверка считала такую строку наличными, каждый обмен
   * вечером превращался в недостачу ровно на цену зачтённого изделия,
   * и продавца обвиняли в воровстве за честно оформленный обмен.
   *
   * По умолчанию 1: все прежние записи — обычные платежи, они через ящик
   * и проходили.
   */
