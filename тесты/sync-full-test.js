...
async function main() {
  ...
  try {
     ...
     *
     * Зазор между записью и дописыванием намеренно КОРОТКИЙ. Система считает
     * файл готовым, когда его размер не изменился два опроса подряд, то есть
     * через два интервала (здесь 300 мс × 2 = 600 мс). Если дописывать позже
     * этого срока, система вправе забрать файл уже после первой строки —
     * и набор падал бы, обвиняя в потере строки исправно работающую систему.
     * Дописываем внутри одного интервала: два опроса в него не помещаются.
     */
