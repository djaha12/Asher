...
/*
 * Снимок экрана, который не может завалить набор.
 *
 * Снимки нужны человеку — посмотреть, что видел продавец. Проверяют они
 * ничего. Но снимок всей страницы длиной в двадцать тысяч точек на занятой
 * машине изредка не укладывается в отведённое время, и тогда набор про права
 * продавца краснеет из-за картинки. Мы на это уже попались: «Права глазами
 * продавца» падали с обрывом по времени в общем прогоне и проходили
 * поодиночке. Не сделанный снимок — это отсутствующая картинка, и ничего больше.
 */
async function снимок(страница, настройки) {
  try {
    await страница.screenshot({ timeout: 20000, ...настройки });
    return true;
  } catch (e) {
    console.log('  (снимок не сделан: ' + String(e.message).split('\n')[0].slice(0, 80) + ')');
    return false;
  }
}

module.exports = { ...pw, chromium, CHROME, КЛЮЧ, снимок };
