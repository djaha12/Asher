...
async function main() {
  ...
  /*
   * Спрашиваем каталог целиком, а не первую сотню строк. Каталог отдаёт
   * самые свежие изделия первыми, а свежие в общем прогоне — это те, что
   * завели наборы до нас: ювелирная часть, касса, сходимость. Сотня строк
   * на них и уходила, демонстрационный каталог вытеснялся из ответа целиком
   * — и набор падал «на складе не осталось изделий» на складе, где их сотня.
   *
   ...
   * бирки.
   */
  async function свободныеПоТочкам() {
  ...
  /*
   * Точку держим одну, пока на ней есть чем работать, и переходим на самую
   * полную из оставшихся, когда витрина опустела. Раньше здесь был обрыв:
   * точку выбирали один раз навсегда, и упереться в её конец при полной
   * второй точке набор считал поломкой. Демо-данные раскладывают товар
  ...
  async function пополнить(нужно) {
    ...
    if (список.length < нужно) {
      const лучшая = [...карта.entries()].sort((a, b) => b[1].length - a[1].length)[0];
      if (!лучшая || лучшая[1].length < нужно) return false;
      точка = лучшая[0];
      список = лучшая[1];
    }
    склад.length = 0;
  ...
  async function изделие() {
    if (!склад.length && !(await пополнить(1))) {
      throw new Error('на складе не осталось изделий для проверки');
    }
    const p = склад.shift();
    взято.add(p.id);
    return p;
  ...
  /*
   * Комплект система собирает только из изделий одной точки, поэтому пара
   * берётся вместе: иначе первое изделие могло оказаться последним на этой
   * витрине, второе приезжало уже с другой — и комплект не создавался бы
   * по причине, к скидкам отношения не имеющей.
   */
  async function пара() {
    if (склад.length < 2 && !(await пополнить(2))) {
      throw new Error('на складе не осталось двух изделий с одной точки');
    }
    return [await изделие(), await изделие()];
  }
