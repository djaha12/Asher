...
async function main() {
  ...
  async function изделие() {
    while (!склад.length) {
      ...
      /*
       * Берём только демонстрационный каталог (артикулы AS-). Изделия
       * с другими артикулами заводят себе соседние наборы и потом их ищут —
       * а этот набор много продаёт. Один раз он уже съел оба изделия
       * с бриллиантами, и набор про сертификаты упал так, будто сломались
       * бирки.
       */
      const все = ((r.data || {}).items || [])
        .filter(p => p.status === 'in_stock' && p.retail_price > 1000
          && /^AS-/.test(p.sku) && !p.set_id && !взято.has(p.id));
      /*
